...
class ClubAdapter(private var clubList: List<Club>, private val clickItemListener: (Club) -> Unit): RecyclerView.Adapter<ClubAdapter.VHolder>() {
    ...
    class VHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private val clubCV: CardView = itemView.findViewById(ClubItemUI.clubCV)
        fun bind(itemClub: Club, clickItemListener: (Club) -> Unit){
            ...
            val clubImg = BitmapFactory.decodeResource(itemView.context.resources,itemClub.clubLogo)
            Palette.generateAsync(clubImg){
                if (it != null) {
                    clubCV.background.setColorFilter(it.getDominantColor(Color.WHITE),PorterDuff.Mode.SRC_ATOP)
                }
            }
        }
    }
